...
class IbizaBannerPlugin_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'ibiza-banner-plugin'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'ibiza-banner-plugin'); ?></h2>
            <table cellspacing="1" cellpadding="2"><tbody>
            <tr><td><?php _e('System', 'ibiza-banner-plugin'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'ibiza-banner-plugin'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'ibiza-banner-plugin');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'ibiza-banner-plugin'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'ibiza-banner-plugin');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'ibiza-banner-plugin'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'ibiza-banner-plugin') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'ibiza-banner-plugin');
            case 'false':
                return __('false', 'ibiza-banner-plugin');

            case 'Administrator':
                return __('Administrator', 'ibiza-banner-plugin');
            case 'Editor':
                return __('Editor', 'ibiza-banner-plugin');
            case 'Author':
                return __('Author', 'ibiza-banner-plugin');
            case 'Contributor':
                return __('Contributor', 'ibiza-banner-plugin');
            case 'Subscriber':
                return __('Subscriber', 'ibiza-banner-plugin');
            case 'Anyone':
                return __('Anyone', 'ibiza-banner-plugin');
        }
        return $optionValue;
    }
