...
class IbizaTvSchedule_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'ibiza-tv-schedule'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'ibiza-tv-schedule'); ?></h2>
            <table cellspacing="1" cellpadding="2"><tbody>
            <tr><td><?php _e('System', 'ibiza-tv-schedule'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'ibiza-tv-schedule'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'ibiza-tv-schedule');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'ibiza-tv-schedule'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'ibiza-tv-schedule');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'ibiza-tv-schedule'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'ibiza-tv-schedule') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'ibiza-tv-schedule');
            case 'false':
                return __('false', 'ibiza-tv-schedule');

            case 'Administrator':
                return __('Administrator', 'ibiza-tv-schedule');
            case 'Editor':
                return __('Editor', 'ibiza-tv-schedule');
            case 'Author':
                return __('Author', 'ibiza-tv-schedule');
            case 'Contributor':
                return __('Contributor', 'ibiza-tv-schedule');
            case 'Subscriber':
                return __('Subscriber', 'ibiza-tv-schedule');
            case 'Anyone':
                return __('Anyone', 'ibiza-tv-schedule');
        }
        return $optionValue;
    }
